...
def show_face():
    ...
    texture_img = Image.open('') #here your picture to open
    texture = texture_img.load()
    #open file with 3d face object to cover it with texture
    f = open('objects/face.obj', 'r')
    lines = f.read()
    points = []
    textures = []
    screen = Screen(scr_x, scr_y)
    #read each line with numbers of objects
